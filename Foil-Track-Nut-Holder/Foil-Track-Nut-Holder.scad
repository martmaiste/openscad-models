    ...
    difference() {
        union() {
            for (dir = [1, -1]) {
                ...
                // 1. Base platform
                base_w_center = 15.0;
                base_dx = (base_w_center - shaft_width) / 2;
                base_r = (base_dx * base_dx + (nut_length / 2) * (nut_length / 2)) / (2 * base_dx);

                translate([0, nut_y, base_thickness/2])
                    intersection() {
                        translate([base_r - base_w_center/2, 0, 0])
                            cylinder(h=base_thickness + 0.1, r=base_r, center=true);
                        translate([-base_r + base_w_center/2, 0, 0])
                            cylinder(h=base_thickness + 0.1, r=base_r, center=true);
                        cube([base_w_center + 1, nut_length, base_thickness], center=true);
                    }
